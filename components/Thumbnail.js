    ...
    return (
        <div ref={ref} className="p-2 group cursor-pointer transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50">
           ...
           <div className="p-2">
               ...
               <p className="opacity-0 group-hover:opacity-100 flex items-center">{result.media_type && `${result.media_type} •`}{" "}{result.release_date || result.first_air_date} •{" "}<ThumbUpIcon className="h-5 mx-2"/>{" "}{result.vote_count} </p>
           </div>
        </div>
    )
})
